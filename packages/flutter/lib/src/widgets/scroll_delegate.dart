...
abstract class TwoDimensionalChildDelegate extends ChangeNotifier {
  ...
  /// to a different position. It should return the indices (represented as an
  /// [Offset]) of the child element with associated key, null if not found.
  ///
  /// If not provided, a child widget may not map to its existing [RenderObject]
  /// when the order of children returned from the children builder changes.
  /// This may result in state-loss.
  ...
  /// widgets have changed, a new delegate must be provided, and the new
  /// delegate's [shouldRebuild] method must return true.
  Widget build(BuildContext context, int yIndex, int xIndex) {
    Widget child = buildChildFor(context, yIndex, xIndex);
    final Key? key = child.key != null ? _SaltedValueKey(child.key!) : null;
    if (addRepaintBoundaries) {
      child = RepaintBoundary(child: child);
    }
  ...
  ///
  Widget buildChildFor(BuildContext context, int yIndex, int xIndex);
