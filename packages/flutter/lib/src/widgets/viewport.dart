...
class _TwoDimensionalViewportElement extends RenderObjectElement
  ...
  Map<ChildIndex, Element> _indexToChild = <ChildIndex, Element>{};
  Map<Key, Element> _keyToChild = <Key, Element>{};
  // Used between startLayout() & endLayout() to compute the new values for
  // _indexToChild and _keyToChild.
  Map<ChildIndex, Element>? _newIndexToChild;
  ...
  @override
  void insertRenderObjectChild(RenderBox child, ChildIndex slot) {
    renderObject.insertChild(child, slot);
  }

  @override
  void moveRenderObjectChild(RenderBox child, ChildIndex oldSlot, ChildIndex newSlot) {
    renderObject.moveChild(child, from: oldSlot, to: newSlot);
  }

  @override
  void removeRenderObjectChild(RenderBox child, ChildIndex slot) {
  ...
  int _compareChildren(Element a, Element b) {
    final ChildIndex aSlot = a.slot! as ChildIndex;
    final ChildIndex bSlot = b.slot! as ChildIndex;
  ...
  Element? _retrieveOldElement(Widget newWidget, ChildIndex index) {
    if (newWidget.key != null) {
