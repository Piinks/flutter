...
abstract class ScrollPosition extends ViewportOffset with ScrollMetrics {
  ...
  Future<void> ensureVisible(
    ...
    double target;
    print('$alignmentPolicy, $minScrollExtent, $maxScrollExtent');
    switch (_applyAxisDirectionToAlignmentPolicy(alignmentPolicy)) {
      case ScrollPositionAlignmentPolicy.explicit:
        target = clampDouble(
          viewport.getOffsetToReveal(object, alignment, rect: targetRect, axisDirection: axisDirection).offset,
          minScrollExtent,
          maxScrollExtent,
        );
      case ScrollPositionAlignmentPolicy.keepVisibleAtEnd:
        target = clampDouble(
          viewport.getOffsetToReveal(object, 1.0, rect: targetRect, axisDirection: axisDirection).offset,
          minScrollExtent,
          maxScrollExtent,
        );
        if (target < pixels) {
      ...
      case ScrollPositionAlignmentPolicy.keepVisibleAtStart:
        target = clampDouble(
          viewport.getOffsetToReveal(object, 0.0, rect: targetRect, axisDirection: axisDirection).offset,
          minScrollExtent,
          maxScrollExtent,
        );
        if (target > pixels) {
    ...
    print('target: $target, pixels: $pixels');
